...
def format_full_trending_email(data):
    ...
    # Generate unique ID to prevent Gmail from folding content
    unique_id = f"{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}{random.randint(1000,9999)}"
    ...
    html += f'<h1 style="text-align:center;color:#24292e;margin-bottom:30px;font-size:28px;">🔥 Tech Trending Daily</h1>'
    
    # Add invisible unique marker to prevent Gmail folding
    html += f'<span style="display:none;font-size:0;color:transparent;">{unique_id}-start</span>'
    ...
    # Add unique marker between sections
    html += f'<span style="display:none;font-size:0;color:transparent;">{unique_id}-hf</span>'
    
    if data.get('huggingFaceModels'):
        html += format_huggingface_models(data['huggingFaceModels'])
    
    html += f'<span style="display:none;font-size:0;color:transparent;">{unique_id}-hn</span>'
    
    if data.get('hackerNewsStories'):
        html += format_hackernews_stories(data['hackerNewsStories'])
    
    html += f'<span style="display:none;font-size:0;color:transparent;">{unique_id}-dev</span>'
    
    if data.get('devToArticles'):
        html += format_devto_articles(data['devToArticles'])
    
    html += f'<span style="display:none;font-size:0;color:transparent;">{unique_id}-ai</span>'
    
    if data.get('aiPapers'):
        html += format_ai_papers(data['aiPapers'])
    
    html += f'<span style="display:none;font-size:0;color:transparent;">{unique_id}-indie</span>'
    
    if data.get('indieRevenue'):
        html += format_indie_revenue(data['indieRevenue'])
    
    html += f'<span style="display:none;font-size:0;color:transparent;">{unique_id}-end</span>'
